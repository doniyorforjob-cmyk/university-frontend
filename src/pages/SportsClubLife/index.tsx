...
import { fetchSportsClubLife } from '@/services/sportsClubLifeService';
import { SportsClubEntry } from '@/api/http/sportsClubLife.http';
import DetailTemplate from '@/components/templates/DetailTemplate';
import DocumentList from '@/components/shared/DocumentList';
import PageSkeleton from '@/components/shared/PageSkeleton';
import EmptyState from '@/components/shared/EmptyState';

const SportsClubLifePage: React.FC = () => {
    const { locale } = useLocale();
    const { t } = useTranslation(['common', 'pages']);
    const { setSidebarType, setBreadcrumbsData } = useGlobalLayout();

    const { data: items, loading, error } = useStandardPage<SportsClubEntry[]>(
        'sports-club-life-list',
        fetchSportsClubLife
    );
    ...
    useEffect(() => {
        ...
        setBreadcrumbsData([
            ...
            { label: t('pages:sportsClubLife', 'Sport klubi hayoti') }
        ]);
    ...
    if (error || !items || items.length === 0) {
        return (
            <DetailTemplate
                title={t('pages:sportsClubLife', 'Sport klubi hayoti')}
                contentType="info"
                showSidebar={false}
            >
    ...
    return (
        ...
        >
            <div className="mt-8 space-y-12">
                {items.map((entry: SportsClubEntry, index: number) => (
...
export default SportsClubLifePage;

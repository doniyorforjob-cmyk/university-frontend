    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
        {photos.slice(0, 5).map((photo: any, index: number) => {
          ...
          return (
            <div key={photo.id} className={index === 4 ? 'hidden xl:block' : ''}>
              <MediaCard
                type="photo"
                title={photo.title}
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
        ...
        {videos.slice(0, 5).map((video: any, index: number) => (
          <div key={video.id} onClick={() => setActiveVideo(index)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setActiveVideo(index); }} className={index === 4 ? 'hidden xl:block' : ''}>
            <MediaCard
              type="video"
              title={video.title}
    ...
    <section className="py-12 md:py-20 lg:py-24 bg-gradient-to-b from-blue-50/30 to-white">
      <Container>
        <MediaGalleryHeader
          ...
          seeAllLink={activeTab === 'photos' ? '/photos' : '/videos'}
        />

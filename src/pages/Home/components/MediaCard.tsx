...
const MediaCardContent: React.FC<MediaCardContentProps> = (props) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="g-card-image relative overflow-hidden rounded-xl md:rounded-2xl shadow-sm transition-all duration-300">
        <AspectRatio ratio={1}>
          <img
            src={thumbnail}
            alt={title}
        ...
        {/* Floating Icon Badge */}
        <div className="absolute top-3 left-3 bg-[#5a5a5a]/60 backdrop-blur-md border border-white/10 rounded-lg p-2 z-20 shadow-lg">
          {type === 'video' ? (
            <PlayIcon className="w-5 h-5 md:w-6 md:h-6 text-white drop-shadow-md" />
          ) : (
            <PhotoIcon className="w-5 h-5 md:w-6 md:h-6 text-white drop-shadow-md" />
          )}
        </div>

        {/* Metadata Overlay - Glassmorphism */}
        <div className="absolute bottom-3 left-3 right-3 bg-[#5a5a5a]/60 backdrop-blur-md border border-white/10 rounded-lg p-2 z-10 shadow-md">
          <div className="flex justify-between items-center text-[#FFE082] w-full gap-2 font-bold text-xs md:text-sm px-1">
            <span className="flex items-center gap-1.5">
              <CalendarDaysIcon className="w-4 h-4 md:w-5 md:h-5 stroke-[2]" />
              {date}
            </span>
            <div className="flex items-center gap-4">
              {photosCount > 0 && (
                <span className="flex items-center gap-1.5 text-[#FFE082]">
                  <PhotoIcon className="w-4 h-4 md:w-5 md:h-5 stroke-[2]" />
                  {photosCount}
                </span>
              )}
              <span className="flex items-center gap-1.5">
                <EyeIcon className="w-4 h-4 md:w-5 md:h-5 stroke-[2]" />
                {views}
              </span>
            </div>
          </div>

...
import { fetchSpiritualActivities } from '@/services/spiritualEducationalService';
import { SpiritualActivityEntry } from '@/api/http/spiritualEducational.http';
import DetailTemplate from '@/components/templates/DetailTemplate';
import DocumentList from '@/components/shared/DocumentList';
import PageSkeleton from '@/components/shared/PageSkeleton';
import EmptyState from '@/components/shared/EmptyState';

const SpiritualEducationalPage: React.FC = () => {
    const { locale } = useLocale();
    const { t } = useTranslation(['common', 'pages']);
    const { setSidebarType, setBreadcrumbsData } = useGlobalLayout();

    const { data: items, loading, error } = useStandardPage<SpiritualActivityEntry[]>(
        'spiritual-educational-list',
    ...
    useEffect(() => {
        ...
        setBreadcrumbsData([
            ...
            { label: t('pages:spiritualEducational', "Ma'naviy-ma'rifiy rukn") }
        ]);
    ...
    if (error || !items || items.length === 0) {
        return (
            <DetailTemplate
                title={t('pages:spiritualEducational', "Ma'naviy-ma'rifiy rukn")}
                contentType="info"
                showSidebar={false}
            >
    ...
    return (
        ...
        >
            <div className="mt-8 space-y-12">
                {items.map((entry: SpiritualActivityEntry, index: number) => (
...
export default SpiritualEducationalPage;

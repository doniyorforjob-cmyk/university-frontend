...
import { fetchScientificActivities } from '@/services/scientificActivityService';
import { ScientificActivityEntry } from '@/api/http/scientificActivity.http';
import DetailTemplate from '@/components/templates/DetailTemplate';
import DocumentList from '@/components/shared/DocumentList';
import PageSkeleton from '@/components/shared/PageSkeleton';
import EmptyState from '@/components/shared/EmptyState';

const ScientificActivityPage: React.FC = () => {
    const { locale } = useLocale();
    const { t } = useTranslation(['common', 'pages']);
    const { setSidebarType, setBreadcrumbsData } = useGlobalLayout();

    const { data: items, loading, error } = useStandardPage<ScientificActivityEntry[]>(
        'scientific-activity',
    ...
    useEffect(() => {
        ...
        setBreadcrumbsData([
            ...
            { label: t('pages:scientificActivity', 'Ilmiy faoliyat') }
        ]);
    ...
    if (error || !items || items.length === 0) {
        return (
            <DetailTemplate
                title={t('pages:scientificActivity', 'Ilmiy faoliyat')}
                contentType="info"
                showSidebar={false}
            >
    ...
    // Displaying all entries as sections
    return (
        <DetailTemplate
            title={t('pages:scientificActivity', 'Ilmiy faoliyat')}
            contentType="info"
        ...
        >
            <div className="mt-8 space-y-12">
                {items.map((entry: ScientificActivityEntry, index: number) => (
...
export default ScientificActivityPage;

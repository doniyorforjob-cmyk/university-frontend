...
const LeadershipCard: React.FC<LeadershipCardProps> = ({ member, isMain = false, variant = 'default', showDuties = true }) => {
    ...
    const getMemberPath = (member: Leadership, locale: string) => {
        const prefix = locale === 'uz' ? '' : `/${locale}`;
        const slug = member.id || member.slug;

        switch (member.category as any) {
            case 'dean':
            case 'academic_head':
                return `${prefix}/leadership/${member.id}`;
            case 'center':
                return `${prefix}/centers/${slug}`;
            case 'admin':
            case 'section':
                return `${prefix}/sections/${slug}`;
            default:
                return `${prefix}/leadership/${slug}`;
        }
    };
        ...
        >
            <div className={`flex flex-col lg:flex-row relative ${isMain ? 'lg:h-[18.2rem]' : 'lg:h-64'} overflow-hidden ${!isMain ? 'lg:flex-row-reverse' : ''}`}>
                ...
                >
                    ...
                    >
                        <h1
                            className={`font-black text-[#003B5C] group-hover:text-blue-600 leading-tight tracking-tight p-0 m-0 ${variant === 'small' ? 'text-2xl md:text-[1.55rem]' : 'text-3xl md:text-[1.8rem]'}`}
                            style={{
                                marginTop: '0px',
                                paddingTop: '0px',
                                marginBottom: '8px',
                ...
                <div className={`relative ${isMain ? 'lg:w-[35%] lg:-ml-8' : 'lg:w-[28%]'} h-72 ${isMain ? 'lg:h-auto' : 'lg:h-72'} overflow-hidden bg-gray-50 flex items-start justify-center z-0 pt-0 mt-0`}>
                    ...
                    >
                        {member.image ? (
                            ...
                            <div className="relative z-20 flex items-center justify-center w-full bg-gray-50 h-full p-8">
                                <img
                                    src={logoUrl}
                                    alt="Logo"
                                    className="w-full max-w-[120px] h-auto object-contain opacity-60"
                    ...
                    >
                        <div className="p-10 prose prose-lg prose-main max-w-none">
                            {expandedSection === 'career' ? (
                                ...
                                showDuties && <div dangerouslySetInnerHTML={{ __html: member.description || '' }} />
                            )}
                        </div>
                    </motion.div>
                )}

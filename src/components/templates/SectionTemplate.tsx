...
const SectionTemplate: React.FC<SectionTemplateProps> = ({
  ...
  // Sana formatini o'zgartiruvchi funksiya — locale aware
      ...
      >
        ...
        <div className="overflow-hidden relative block">
          ...
          {showCategoryBadge && item.category && (
            <div
              className="absolute top-3 left-3 text-white text-[10px] font-black px-2.5 py-1 rounded-md uppercase tracking-wider z-10 shadow-sm border border-white/10"
              style={{
                backgroundColor:
                  item.categoryType === 'news' ? '#2563eb' : // Blue
                    item.categoryType === 'announcements' ? '#ea580c' : // Orange
                      item.categoryType === 'openLessons' ? '#16a34a' : // Green
                        item.categoryType === 'media' ? '#9333ea' : // Purple
                          '#4b5563' // Default Gray
              }}
            >
              {item.category}
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          <div className="flex justify-between items-center text-sm text-gray-500 mb-2">
            ...
            <div className="flex items-center">
              ...
              <span className="text-gray-600 font-medium">{((item.image ? 1 : 0) + ((item as any).gallery?.length || 0))}</span>
            </div>
          </div>
  ...
  if (loading && items.length === 0) {
    const skeletonLayout = (layoutType === 'list' || layoutType === 'horizontal') ? 'list' :
      (layoutType === 'grid' || layoutType === 'card' || layoutType === 'masonry') ? 'grid' : 'default';

    return <GenericPageSkeleton layoutType={skeletonLayout} gridItems={itemsPerPage} showSidebar={false} showBanner={false} noContainer={true} />;
  }
            ...
            {children ? (
              ...
              >
                {currentItems.map((item, index) => (
                  ...
                  >
                    {customRenderItem ? customRenderItem(item, index) : renderItem(item)}
                  </motion.div>
                ))}
              </motion.div>
            ) : (

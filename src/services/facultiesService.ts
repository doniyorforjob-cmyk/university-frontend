...
export const getFaculties = useMock
    ? async (locale?: string): Promise<Faculty[]> => mockFacultiesData
    : httpGetFaculties;

export const getFacultyById = useMock
    ? async (id: string | number, locale?: string): Promise<Faculty | null> => mockFacultiesData.find(f => String(f.id) === String(id)) || null
    : httpGetFacultyById;

export const getDepartments = useMock
    ? async (locale?: string): Promise<Department[]> => mockDepartmentsData
    : httpGetDepartments;

export const getDepartmentById = useMock
    ? async (id: string | number, locale?: string): Promise<Department | null> => mockDepartmentsData.find(d => String(d.id) === String(id)) || null
    : httpGetDepartmentById;

export const getDepartmentsByFacultyId = useMock
    ? async (facultyId: string | number, locale?: string): Promise<Department[]> => mockDepartmentsData.filter(d => String(d.facultyId) === String(facultyId))

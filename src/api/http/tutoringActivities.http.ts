...
export interface TutoringActivityFile {
    name: string;
    url: string;
    size?: number;
    ext?: string;
...
export interface TutoringActivityEntry {
    id: string;
    slug: string;
    title: string;
    content?: string;
    files: TutoringActivityFile[];
    date?: string;
    image?: string;
    gallery?: string[];
}

export const fetchTutoringActivities = async (locale?: string): Promise<TutoringActivityEntry[]> => {
    try {
        const projectId = process.env.REACT_APP_PROJECT_ID;
        const response = await apiClient.get(`/projects/${projectId}/content/tutoring-activities`, {
            params: {
                locale,
        ...
        console.error('Error fetching tutoring activities:', error);
        return [];
    }
};

export const getTutoringActivityBySlug = async (slug: string, locale?: string): Promise<TutoringActivityEntry | null> => {
    try {
        const projectId = process.env.REACT_APP_PROJECT_ID;
        ...
        console.error(`Error fetching tutoring activity by slug: ${slug}`, error);
        return null;
    }
};

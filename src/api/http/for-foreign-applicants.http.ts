...
export interface ForeignApplicantFile {
    name: string;
    url: string;
    size?: number;
    ext?: string;
...
export interface ForeignApplicantEntry {
    id: string;
    slug: string;
    title: string;
    content: string;
    published_at: string;
    files: ForeignApplicantFile[];
}

export const fetchForForeignApplicantsData = async (locale?: string): Promise<ForeignApplicantEntry | null> => {
    try {
        const projectId = process.env.REACT_APP_PROJECT_ID;
        const response = await apiClient.get(`/projects/${projectId}/content/for-foreign-applicants`, {
            params: {
                locale,
                with: 'files'
            }
        ...
        console.error('For foreign applicants fetch error:', error);
        return null;
    }
};

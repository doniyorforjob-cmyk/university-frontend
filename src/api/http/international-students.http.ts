...
export interface InternationalStudentFile {
    name: string;
    url: string;
    size?: number;
    ext?: string;
...
export interface InternationalStudentEntry {
    id: string;
    slug: string;
    title: string;
    content: string;
    ...
    published_at: string;
    views: number;
    files: InternationalStudentFile[];
}

export const fetchInternationalStudentsData = async (locale?: string): Promise<InternationalStudentEntry | null> => {
    try {
        const projectId = process.env.REACT_APP_PROJECT_ID;
        const response = await apiClient.get(`/projects/${projectId}/content/international-students`, {
            params: {
                locale,
        ...
        console.error("International students fetch error:", error);
        return null;
    }
};

...
export interface PublicCouncilData {
    title: string;
    slug?: string;
    content?: string;
...
export const getPublicCouncilData = async (locale: string = 'uz'): Promise<PublicCouncilData | null> => {
    try {
        const projectId = process.env.REACT_APP_PROJECT_ID;

        const response = await apiClient.get(`/projects/${projectId}/content/public-council`, {
            params: { locale }
        });
        ...
        console.error('Error fetching public council data:', error);
        return null;
    }
};

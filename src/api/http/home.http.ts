...
export const homeApi = {
  ...
  getNewsData: async (locale?: string): Promise<any> => {
    return fetchWithFallback('news', { with: 'image,gallery', per_page: 30 }, locale, true);
  },
  ...
  getAnnouncementsData: async (locale?: string): Promise<any> => {
    return fetchWithFallback('announcements', { with: 'image,gallery', per_page: 30 }, locale, true);
  },

  getCombinedNewsData: async (locale?: string): Promise<any> => {
    try {
      const [news, events, announcements, corruption] = await Promise.all([
        fetchWithFallback('news', { with: 'image,gallery', per_page: 30 }, locale, true).catch(() => []),
        fetchWithFallback('events', { with: 'image,gallery', per_page: 30 }, locale, true).catch(() => []),
        fetchWithFallback('announcements', { with: 'image,gallery', per_page: 30 }, locale, true).catch(() => []),

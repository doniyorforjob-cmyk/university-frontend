...
export interface ScientificActivityEntry {
    id: string;
    slug: string;
    title: string;
    content?: string;
...
export const fetchScientificActivities = async (locale?: string): Promise<ScientificActivityEntry[]> => {
    try {
        const projectId = process.env.REACT_APP_PROJECT_ID;
        const response = await apiClient.get(`/projects/${projectId}/content/scientific-activity`, {
            params: {
                locale,
                with: 'files'
            }
        ...
        console.error('Error fetching scientific activities:', error);
        return [];
    }
};
